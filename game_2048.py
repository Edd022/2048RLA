...
def guardar_resultado(tablero, score):
    max_tile = int(tablero.max())
    gano = int(max_tile >= 2048)
    with open("resultados_2048.csv", mode="a", newline="") as file:
        writer = csv.writer(file)
        writer.writerow([gano, max_tile, score])
...
while True:
    ...
    if auto_play:
        ...
        action = agent.select_action(env.game)
    ...
    if done:
        ...
        guardar_resultado(env.game.get_board(), score)
